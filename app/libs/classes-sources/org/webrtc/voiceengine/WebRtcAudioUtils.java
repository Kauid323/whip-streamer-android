...
/* loaded from: classes.jar:org/webrtc/voiceengine/WebRtcAudioUtils.class */
public final class WebRtcAudioUtils {
    ...
    public static synchronized void setWebRtcBasedAutomaticGainControl(boolean enable) {
        Logging.m3w(TAG, "setWebRtcBasedAutomaticGainControl() is deprecated");
    }

    public static synchronized boolean useWebRtcBasedAcousticEchoCanceler() {
        if (useWebRtcBasedAcousticEchoCanceler) {
            Logging.m3w(TAG, "Overriding default behavior; now using WebRTC AEC!");
        }
    ...
    public static synchronized boolean useWebRtcBasedNoiseSuppressor() {
        if (useWebRtcBasedNoiseSuppressor) {
            Logging.m3w(TAG, "Overriding default behavior; now using WebRTC NS!");
        }
    ...
    static void logDeviceInfo(String tag) {
        Logging.m1d(tag, "Android SDK: " + Build.VERSION.SDK_INT + ", Release: " + Build.VERSION.RELEASE + ", Brand: " + Build.BRAND + ", Device: " + Build.DEVICE + ", Id: " + Build.ID + ", Hardware: " + Build.HARDWARE + ", Manufacturer: " + Build.MANUFACTURER + ", Model: " + Build.MODEL + ", Product: " + Build.PRODUCT);
    }
    ...
    private static void logAudioStateBasic(String tag, AudioManager audioManager) {
        Logging.m1d(tag, "Audio State: audio mode: " + modeToString(audioManager.getMode()) + ", has mic: " + hasMicrophone() + ", mic muted: " + audioManager.isMicrophoneMute() + ", music active: " + audioManager.isMusicActive() + ", speakerphone: " + audioManager.isSpeakerphoneOn() + ", BT SCO: " + audioManager.isBluetoothScoOn());
    }
    ...
    private static void logAudioStateVolume(String tag, AudioManager audioManager) {
        ...
        Logging.m1d(tag, "Audio State: ");
        boolean fixedVolume = isVolumeFixed(audioManager);
        Logging.m1d(tag, "  fixed volume=" + fixedVolume);
        if (!fixedVolume) {
    ...
    private static String deviceTypeToString(int type) {
        switch (type) {
            ...
            case org.webrtc.audio.WebRtcAudioRecord.DEFAULT_AUDIO_SOURCE /* 7 */:
